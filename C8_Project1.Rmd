...
In order to fall within the length limitations some text and figure output is
suppressed in this report; when applicable this is noted at the beginning of the
code chunk and the output is described as comments. Additionally, 2 functions
were written to facilitate data processing, and the code for these functions is
not shown either. For those interested, unabriged analysis can be viewed in the
[GitHub repo][2] for this project submission.

[2]: xxxurlxxx "GitHub repo"
...
In the next code chunk the data are previewed with `str()`, and it can be seen
that some factor variables like `user_name` and `classe` were loaded as
character type and need to be converted. Also, beginning at column 12 some
numeric variables were loaded as character type and also need conversion. It can
also be seen that many of these values are simply blank spaces. A function was
created to do the type conversions and easily repeat it later on the validation
and test sets. When the blank spaces are converted to numeric it introduces NAs
by coercion and these warnings are suppressed in the function.
...
summary(train.subset[1:15])  # Some columns have NAs
# Check for NA values
...
these columns is over 97%. By reading the research paper, one can find that the
authors defined "windows" of data (several sequential observations) and
...
possibly contained variables that would cause problems or unnecessary
complication in the model fitting step. The remaining variables were assessed,
and it was determined that six columns could be removed from the training data.
Additional detail on the justification for these eliminations can be found in
the script for this project in the [GitHub repo][2]. As the unneeded variables
were removed in the chunk above, the data are in tidy format and ready for model
fitting.
...
In the final code chunk below, the testing data downloaded and then processed in
the same way as the training data. The model is then used to predict on the test
data, and the prediction results are shown below.
